...
- Retrofit, not rebuild: the pegasus-monitord entry-point plugin system (pegasus branch `monitord-plugin-system`, pinned by `MONITORD_PLUGIN_REF`, currently `317a020a3…`) is a 3-file overlay onto the apt-installed Pegasus + the `wfmonitor` adapter (workflow-monitor branch `monitord-plugin-adapter`). Install once with `--install-monitord-plugin`; opt in per run with `--enable-monitord-plugin`.
...
- Standalone `--enable-wfevents-plugin` writes `wfevents.jsonl` (same schema) consumed via `workflow-monitor --remote` — Vector does NOT tail it; install once from `mjstealey/pegasus-monitord-plugins`. `--wfevents-condor-poll` lets it poll condor; enable polling on ONE plugin only (pair with `--no-monitord-condor-poll` if `wfmonitor` is also on).
...
- FABlib config precedence: constructor arg ▸ `fabric_rc` ▸ env ▸ defaults. It does NOT read `FABRIC_RC`; with no arg it reads `~/work/fabric_config/fabric_rc` — hence the explicit `fabric_rc=`.
- `FABRIC_AVOID` must be a plain comma list (`EDUKY,EDC,...`), not Python-list syntax — FABlib `.split(",")`s the raw value; brackets/quotes silently fail to avoid sites.
- Paths in `config/fabric_rc` are absolute and host-specific; moving machines means changing every path and `FABRIC_BASTION_USERNAME`.

## Don't
- Don't `source config/fabric_rc` — `FABRIC_SSH_COMMAND_LINE` / `FABRIC_AVOID` contain spaces/braces that break shell word-splitting. Use `FablibManager(fabric_rc=…)` or `Config(fabric_rc=…, offline=True)` instead.
